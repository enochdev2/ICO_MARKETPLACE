...
const Header = ({
  ...
  useEffect(() => {
    if (typeof window.ethereum !== 'undefined') {
      ...
      return () => {
        if (typeof window.ethereum !== 'undefined') {
          window.ethereum.removeListener("accountsChange", handleAccountsChanged);
        }
      }
    }
  }, [address]);
  ...
  return (
    ...
    >
      <div className="flex items-center px-5 lg:px-7.5 xl:px-10 max-lg:py-4">
        <a className="flex justify-center items-center text-2xl font-bold space-x-3 w-[12rem] xl:mr-8" href="">
      ...
      <li className=" relative font-code text-2xl uppercase text-n-1 transition-colors hover:text-color-1 px-4 py-6 md:py-8 lg:-mr-0.25 lg:text-sm font-bold lg:font-semibol lg:leading-5 lg:hover:text-n-1 xl:px-6 cursor-pointer">
        <a onClick={() => openCreateICO ? setOpenCreateICO(false) 
          : setOpenCreateICO(true)}>
             Create ICO
          </a>
      </li>
      <li  className=" relative font-code text-2xl uppercase text-n-1 transition-colors hover:text-color-1 px-4 py-6 md:py-8 lg:-mr-0.25 lg:text-sm font-bold lg:font-semibol lg:leading-5 lg:hover:text-n-1 xl:px-6 cursor-pointer">
        <a href="/#history" onClick={() => openTokenHIstory ? setOpenTokenHistory(false) 
          : setOpenTokenHistory(true)}>
             HIstory
          </a>
      </li>
      <li className=" relative font-code text-2xl uppercase text-n-1 transition-colors hover:text-color-1 px-4 py-6 md:py-8 lg:-mr-0.25 lg:text-sm font-bold lg:font-semibol lg:leading-5 lg:hover:text-n-1 xl:px-6 cursor-pointer">
        <a onClick={() => openTokenCreator ? setOpenTokenCreator(false) 
        ...
        name={`${shortenAddress(address)} : ${accountBalance?.slice(0,5)}`}
          ...
          <button onClick={connectWallet} className="button relative  items-center justify-center h-11 transition-colors hover:text-color-1 hidden lg:flex text-black px-7  bg-[#ffee55] rounded-lg" >
      Connect Wallet
    </button>
        )
      }
